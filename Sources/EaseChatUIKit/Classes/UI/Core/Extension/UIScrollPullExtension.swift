...
extension UIScrollView {
    ...
    public func addPullToRefreshWithAction(_ action: @escaping (() -> ())) {
        let pullRefresher = ShengwangChatUIKit.PullRefresher(action: action, frame: CGRect(x: 0, y: -pullRefreshDefaultHeight, width: self.frame.size.width, height: pullRefreshDefaultHeight))
    ...
    public func addPullToRefreshWithAction(_ action: @escaping (() -> ()), withAnimator animator: PullRefresherDelegate, withSubview subview: UIView) {
        ...
        let pullRefresher = ShengwangChatUIKit.PullRefresher(action: action, frame: CGRect(x: 0, y: -height, width: self.frame.size.width, height: height), animator: animator, subview: subview)
        pullRefresher.tag = pullRefreshTag
        self.addSubview(pullRefresher)
    }
    ...
    public func addPullToRefreshWithAction<T: UIView>(_ action: @escaping (() -> ()), withAnimator animator: T) where T: PullRefresherDelegate {
        ...
        let pullRefresher = ShengwangChatUIKit.PullRefresher(action: action, frame: CGRect(x: 0, y: -height, width: self.frame.size.width, height: height), animator: animator, subview: animator)
        pullRefresher.tag = pullRefreshTag
        self.addSubview(pullRefresher)
    }
