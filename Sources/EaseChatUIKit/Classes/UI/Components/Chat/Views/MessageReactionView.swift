...
@objcMembers open class MessageReactionView: UIView {
    ...
    public private(set) lazy var reactionMenus: UICollectionView = {
        ...
        return UICollectionView(frame: CGRect(x: 0, y: 0, width: self.frame.width-30, height: self.frame.height), collectionViewLayout: flow).delegate(self).dataSource(self).registerCell(MessageReactionCell.self, forCellReuseIdentifier: "ShengwangChatUIKit.MessageReactionCell").backgroundColor(.clear).showsVerticalScrollIndicator(false).showsHorizontalScrollIndicator(false)
    }()
...
extension MessageReactionView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "ShengwangChatUIKit.MessageReactionCell", for: indexPath) as? MessageReactionCell
        if let reaction = self.datas[safe: indexPath.row] {
