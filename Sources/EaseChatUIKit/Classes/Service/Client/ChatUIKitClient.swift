...
public let ChatUIKit_VERSION = "2.0.1"
...
@objcMembers public class UIOptions: NSObject {
    ...
    /// Whether to store session avatars and nicknames in ShengwangChatUIKit.
...
@objcMembers public class ChatUIKitClient: NSObject {
    ...
    @objc(setupWithAppId:option:)
    public func setup(appId: String? = nil,option: ChatOptions? = nil) -> ChatError? {
        if let options = option {
            self.option = ChatUIKitOptions()
            self.option.option_chat = options
            return ChatClient.shared().initializeSDK(with: self.option.option_chat)
        }
        return ChatClient.shared().initializeSDK(with: ChatOptions(appId: appId ?? ""))
    }
