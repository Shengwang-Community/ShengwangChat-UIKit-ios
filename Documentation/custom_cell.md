...
## 1.根据需求继承`ShengwangChatUIKit`中的Cell
...
## 2.根据需求继承`ShengwangChatUIKit`中的Cell的渲染模型`MessageEntity`，并给定气泡大小，其中`redPackageIdentifier`为红包的自定义消息的event事件
...
## 5.给`ShengwangChatUIKit`中的`MessageListViewModel`增加发送红包消息的方法
...
## 6.将上述继承的对象全部注册进`ShengwangChatUIKit`，在其初始化后
